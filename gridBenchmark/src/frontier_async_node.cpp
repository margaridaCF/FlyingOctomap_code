...
void frontier_request_callback(const octomap_msgs::Octomap::ConstPtr& octomapBinary){
  octree = (octomap::OcTree*)octomap_msgs::binaryMsgToMap(*octomapBinary);
  got_first_octomap = true;
  // ROS_INFO("[Frontier] Got first message!");
}
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber frontier_request_sub = nh.subscribe<mavros_msgs::State>("frontier_request", 10, state_cb);
