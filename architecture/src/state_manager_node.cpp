...
geometry_msgs::TwistStamped calculateVelocity()
{
    ...
    double cruising_speed = 1;
    double magnitude_vec_to_target = sqrt(pow(position_state.pose.position.x - current_position.pose.position.x, 2) +
                                          pow(position_state.pose.position.y - current_position.pose.position.y, 2) +
                                          pow(position_state.pose.position.z - current_position.pose.position.z, 2));

    if (magnitude_vec_to_target >= 3)
    {
        unit_vector.twist.linear.x = (position_state.pose.position.x - current_position.pose.position.x) / magnitude_vec_to_target;
        unit_vector.twist.linear.y = (position_state.pose.position.y - current_position.pose.position.y) / magnitude_vec_to_target;
        unit_vector.twist.linear.z = (position_state.pose.position.z - current_position.pose.position.z) / magnitude_vec_to_target;
    ...
    else if (magnitude_vec_to_target >= 1)
    {
